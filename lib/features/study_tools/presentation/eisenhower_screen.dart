...
class _EisenhowerScreenState extends State<EisenhowerScreen> with TickerProviderStateMixin {
  late TabController _tabController;
  late AnimationController _entryAnimController;

  // --- STATE DATA (RAM Only) ---
  List<EisenhowerTask> tasks = [
    EisenhowerTask(id: '1', title: 'Urgent Bug Fix', description: 'Critical production issue', quadrant: Quadrant.doFirst),
    EisenhowerTask(id: '2', title: 'Learn State Management', description: 'For better architecture', quadrant: Quadrant.schedule),
  ];
  ...
  Color _getQuadrantColor(Quadrant q) {
    switch (q) {
      case Quadrant.doFirst: return AppColors.eisenhowerDo;
      case Quadrant.schedule: return AppColors.eisenhowerPlan;
      case Quadrant.delegate: return AppColors.eisenhowerDelegate;
      case Quadrant.delete: return AppColors.eisenhowerDrop;
    }
  }

  String _getQuadrantLabel(Quadrant q) {
    switch (q) {
      case Quadrant.doFirst: return "Do First";
      case Quadrant.schedule: return "Schedule";
      case Quadrant.delegate: return "Delegate";
      case Quadrant.delete: return "Eliminate";
    }
  }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildGuideItem(Icons.flash_on_rounded, AppColors.eisenhowerDo, "Do First", "Urgent & Important. Do it now."),
                      _buildGuideItem(Icons.calendar_month_rounded, AppColors.eisenhowerPlan, "Schedule", "Not Urgent but Important. Plan a time."),
                      _buildGuideItem(Icons.people_alt_rounded, AppColors.eisenhowerDelegate, "Delegate", "Urgent but Not Important. Can someone else do it?"),
                      _buildGuideItem(Icons.delete_outline_rounded, AppColors.eisenhowerDrop, "Drop", "Neither Urgent nor Important. Eliminate it."),
                    ],
                  ),
                ),
              ),
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: const Text("Start Prioritizing", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                  ),
                ),
              ),
            ],
  ...
  void _showTaskSheet({EisenhowerTask? task}) {
    ...
    Quadrant selectedQuadrant = task?.quadrant ?? Quadrant.values[_tabController.index];
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 24),
                  Text(task == null ? "New Task" : "Edit Task", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppColors.textMain)),
                  ...
                  // Category Selector
                  const Text("Category", style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textMuted, fontSize: 13)),
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        if (titleController.text.trim().isNotEmpty) {
                          if (task == null) {
                            _addTask(titleController.text, descController.text, selectedQuadrant);
                          } else {
                            _editTask(task.id, titleController.text, descController.text, selectedQuadrant);
                          }
  ...
  Widget build(BuildContext context) {
    ...
    final activeColor = _getQuadrantColor(Quadrant.values[_tabController.index]);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Priorities", style: TextStyle(fontWeight: FontWeight.w800, color: AppColors.textMain)),
        actions: [
          IconButton(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.assignment_add, size: 48, color: color.withValues(alpha: 0.5)),
            ),
            const SizedBox(height: 16),
            Text(
              "No tasks in ${_getQuadrantLabel(quadrant)}",
              style: TextStyle(color: AppColors.textMuted.withValues(alpha: 0.6), fontWeight: FontWeight.w600),
            ),
      ...
      itemBuilder: (context, index) {
        ...
        return SlideTransition(
          position: Tween<Offset>(begin: const Offset(0, 0.1), end: Offset.zero).animate(
            CurvedAnimation(parent: _entryAnimController, curve: Interval(index * 0.1, 1.0, curve: Curves.easeOut)),
          ),
          child: Dismissible(
            key: Key(task.id),
                              ...
                              children: [
                                ...
                                GestureDetector(
                                  ...
                                  child: AnimatedContainer(
                                    ...
                                    width: 24, height: 24,
                                    decoration: BoxDecoration(
                                      color: task.isCompleted ? Colors.grey[300] : color.withValues(alpha: 0.1),
                                      shape: BoxShape.circle,
                                      border: Border.all(
                                        color: task.isCompleted ? Colors.grey[400]! : color,
                                        width: 2
                                      ),
                                    ),
                                    ...
                                    children: [
                                      Text(
                                        ...
                                        style: TextStyle(
                                          fontSize: 15,
                                          fontWeight: FontWeight.bold,
                                          color: task.isCompleted ? AppColors.textMuted : AppColors.textMain,
                                      ...
                                      if (task.description.isNotEmpty) ...[
                                        ...
                                        Text(
                                          ...
                                          style: TextStyle(fontSize: 13, color: AppColors.textMuted.withValues(alpha: 0.7)),
                                        ),
                                      ]
                                    ],
                                  ),
